...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.run(sql`CREATE TABLE \`work_gallery\` (
  ...
  `)
  await db.run(sql`CREATE INDEX \`work_gallery_order_idx\` ON \`work_gallery\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`work_gallery_parent_id_idx\` ON \`work_gallery\` (\`_parent_id\`);`)
  await db.run(sql`CREATE INDEX \`work_gallery_image_idx\` ON \`work_gallery\` (\`image_id\`);`)
  await db.run(sql`CREATE TABLE \`work\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  	\`posted_at\` text NOT NULL,
  	\`destination\` text DEFAULT 'page' NOT NULL,
  	\`type\` text,
  	\`statement\` text,
  	\`cover_id\` integer,
  	\`external_url\` text,
  ...
  `)
  await db.run(sql`CREATE INDEX \`work_cover_idx\` ON \`work\` (\`cover_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  await db.run(sql`DROP TABLE \`work_gallery\`;`)
